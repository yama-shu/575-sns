...
| `/admin/reports` | S-13 通報一覧（運営） | ❌ **運営以外は 404** |
...
運営にすると、ナビゲーションに「通報」が出ます。**運営でなければ出しません** —
出し分けないと、運営向けの画面があること自体を教えることになります。

`/admin/reports` では通報された句が句ごとに改行されて出ます。
「問題なし（却下）」は一度で押せますが、**「非表示にする」は畳んであります**。
非表示にすると画面から元に戻す手段が無いためです。
