...
実装フェーズの開始時に、ルート [README.md](../README.md) へ以下を追記します
（現在は設計フェーズのため未記載）。

- [ ] 開発に必要な言語・ツールとそのバージョン
- [ ] 開発環境の構築手順（devcontainer / セットアップスクリプト）
- [ ] ビルド方法
- [ ] ローカル実行方法（`docker compose up` で全サービスが起動すること）
- [ ] 設定方法（環境変数・設定ファイル・コマンドライン引数の一覧と既定値）
- [ ] テスト実行方法（単体・結合・カバレッジ取得）
- [ ] リリース手順（リリース版の作成方法）
