...
> **投稿完了の遷移先を S-02 から S-01 に改めた**（[#52](https://github.com/yama-shu/575-sns/issues/52)）。
> フォロー中タイムラインの問い合わせは `follows` を起点にしているため、
> **自分の投稿はそこに出ない**。実機で確かめている。
>
> ```
> tarou（hanako をフォロー）の home   → [('hanako', '古池や蛙飛び込む水の音')]
> tarou の public                     → [('tarou', '柿くへば…'), ('hanako', '古池や…')]
> ```
>
> 詠んだ直後に何も無い画面へ送ることになるため、詠んだ句が見える方へ送る。
