...
| **自分が**ブロックしている相手 | 422 | `BLOCKED_USER` |
| 相手が存在しない / 退会済み | 404 | `NOT_FOUND` |
| **相手が自分を**ブロックしている | 404 | `NOT_FOUND` |

ブロックは向きによって応答が違う。

自分のブロックは自分が知っているため、`BLOCKED_USER` で理由を返してよい。
一方、**相手にブロックされている側には理由を返さない**。
`BLOCKED_USER` を返すとブロックされた事実が分かり、
[BR-10](02-domain-model.md#関係に関するルール)「ブロックされた側は、ブロックされた事実を知らされない」に反する。
404 にすれば、存在しない識別名・退会済みと区別がつかない。

> **`USER_NOT_FOUND` から `NOT_FOUND` に訂正した（#34）。**
> [詳細設計 03 のエラー一覧](../detail/03-error-handling.md#2-エラー一覧)に `USER_NOT_FOUND` は無く、
> `NOT_FOUND`（対象が存在しない / 削除済み）が定義されている。
> 資源ごとにコードを分けると `POST_NOT_FOUND` / `USER_NOT_FOUND` と増え続け、
