...
E2E テストの設計は [§11](#11-e2e-テストの設計) に置く。
...
> **測定器によって指標が違う。** prosody は `pytest-cov` の `--cov-branch` で
> **分岐網羅**を測れるが、**Go の `cover` は分岐網羅を測れない**。api の値は
> **文網羅**である（[#95](https://github.com/yama-shu/575-sns/issues/95)）。
>
> **api はモジュール全体の目標を定めていない。** 上の表は層ごとの目標であり、
> `go test ./...` の全体の値は、テストを書いていないパッケージ（`cmd/*` など）を
> 分母に含むため、この表と比較できない。CI のコメントも層ごとに表示する。
...
| # | 入力 | 期待 | 狙い |
| --- | --- | :---: | --- |
| TC-MC-15 | `ッ` | 1 | 単独で与えられた促音。結合先が無い場合の扱い |
| TC-MC-16 | `ン` | 1 | 同上（撥音） |
| TC-MC-17 | `ー` | 1 | 同上（長音） |
...
#### 「ッ」を結合対象に入れる誤りは、単独のケースでは検出できない

[詳細設計 01](01-prosody-algorithm.md#r2-で結合する文字) が「最も起きやすいバグ」と
指摘した誤り（`COMBINING` に `ッ` を含めてしまう）は、
**TC-MC-15〜17 では検出できない**。実装が結合先の有無を確認するためである。

```python
if ch in COMBINING and morae:   # ← morae が空なら結合しない
    morae[-1] += ch
```

単独の `ッ` は結合先が無いため、誤りの有無にかかわらず1モーラになる。

```
正しい実装:              誤った実装（ッ を結合対象に含む）:
  'ッ'          = 1        'ッ'          = 1    ← 同じ。検出できない
  'キッ'        = 2        'キッ'        = 1    ← 違う。検出できる
  'シュッチョウ' = 4        'シュッチョウ' = 3    ← 違う。検出できる
```

この誤りを実際に検出するのは、**促音が他の文字に続くケース**である。
...
加えて、結果からの検出とは別に、**集合の定義そのものを固定するテスト**を置く。
こちらは実装の書き方に依存せず誤りを捉える。

```python
assert COMBINING & INDEPENDENT == frozenset()
assert INDEPENDENT == frozenset("ッンー")
```

TC-MC-15〜17 は依然として必要である。ただしその役割は
「最も起きやすいバグの検出」ではなく、**結合先が無い場合の境界の固定**である。
...
**専用のデータベースに対して実行する。** 各テストの前後で
`reports` / `posts` / `users` を全件削除するため、開発用の DB に向けると
手元のデータが消える（[#80](https://github.com/yama-shu/575-sns/issues/80) で実際に消えた）。

歯止めは2つ置く。**片方だけでは足りない。**

| 歯止め | 防ぐもの |
| --- | --- |
| 接続先を `API_TEST_DATABASE_URL` から読む（`API_DATABASE_URL` と分ける） | api が動く場所で `go test` を叩くこと |
| 接続先の名前が `_test` で終わることを確かめる | 新しい変数に開発用の DSN を書くこと |

**CI では未設定を許さない。** 設定を落とすと結合テストが丸ごとスキップされ、
緑のまま何も確かめていない状態になる。`CI` が設定されていて接続先が無ければ失敗させる。

実行方法は [README](../../../README.md#結合テストapi--db) にある。
...
[§1 のテストの配分](#テストの配分)の頂点にあたる。**単体テストと結合テストは各サービスの中で閉じており、
db・prosody・api・web をつないだときに通るかを確かめる場所がない。**

> **本節は §10 の後に置いている。** §8・§9 は他のドキュメントから節番号で参照されており、
> 間に挿し込むと既存のリンクが別の節を指す。
...
| 素の CDP スクリプト | 採らない。待機もアサーションも自前になる |
| Cypress | 採らない。独自の実行環境にテストを寄せる必要がある |
...
| ケース ID | 確かめること | 対応する要求 |
| --- | --- | --- |
| TC-E2E-01 | 登録し、詠み、全体タイムラインに句ごとに改行されて出る | [FR-01-01](../../requirements/01-requirements.md#fr-01-アカウント) / [FR-02-03](../../requirements/01-requirements.md#fr-02-投稿詠む) / [FR-03-06](../../requirements/01-requirements.md#fr-03-閲覧) |
| TC-E2E-02 | 破調は投稿できず、規定との差が出る | [FR-02-04](../../requirements/01-requirements.md#fr-02-投稿詠む) |
| TC-E2E-03 | 未ログインでは詠めない | [FR-01-02](../../requirements/01-requirements.md#fr-01-アカウント) |
| TC-E2E-04 | フォローすると、フォロー中タイムラインと一覧に出る | [FR-04-02](../../requirements/01-requirements.md#fr-04-交流) / [FR-04-03](../../requirements/01-requirements.md#fr-04-交流) |
| TC-E2E-05 | ブロックすると、その人の句が見えなくなる | [FR-05-02](../../requirements/01-requirements.md#fr-05-健全性) / [BR-09](../basic/02-domain-model.md) |

### 本番相当のイメージで実行する

`docker compose -f compose.yaml`（`compose.override.yaml` を読ませない）で立てる。

**開発モードでは測っているものが変わる。** 初回のアクセスでその画面の
コンパイルが走り、判定の往復に数十秒が乗る。実装を疑う原因になった
（[#71](https://github.com/yama-shu/575-sns/issues/71)）。
...
575 は登録に招待もメール確認も要らないため、テストごとに一意な識別名で登録できる。

### 判定はモックしない

**つないだときに通るかを見るのが目的**である。prosody をモックすると、
判定エンジンと画面のつなぎ目という一番確かめたい部分が抜ける。

### 眠って待たない

`waitForTimeout` を使わない。Playwright の `expect(...).toBeVisible()` は
条件が満たされるまで待つ。そちらを使う。

### 並列数は判定エンジンに合わせる

prosody の既定のワーカー数は 1 で（[compose.yaml](../../../compose.yaml)）、
api は 1 秒で見切りをつける（`API_PROSODY_TIMEOUT`）。

既定の並列数（CPU の半分）で走らせたところ、登録の bcrypt と判定が CPU を奪い合い、
**画面に「判定できません」（504）が出て落ちた**。並列数を 2 に抑える。
