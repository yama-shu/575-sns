...
```mermaid
flowchart TD
    A["形態素を受け取る"] --> Z{"表層は<br/>記号・空白か"}
    Z -->|そう| F["0モーラとして無視"]
    Z -->|違う| D0{"表層は数字か"}
    D0 -->|そう| D["数値読み変換<br/>2024 → ニセンニジュウヨン"]
    D0 -->|違う| B{"解析器の読みは<br/>カタカナか"}
    B -->|カタカナ| OK["その読みを使う"]
    B -->|そうでない| E0{"表層はラテン文字か"}
    E0 -->|そう| E["アルファベット読み<br/>AI → エーアイ"]
    E0 -->|違う| G["読めない語として記録"]
    D --> OK
    E --> OK
    F --> OK
...
**分岐は「読みが空か」ではなく「読みがカタカナか」である。**

SudachiPy は読みを付けられなかった語に対し、表層をそのまま読みとして返す
（`彁` → `彁`）。「空か」で判定すると、これを読めたものとして扱い、
表層の文字数がモーラ数に化ける。`unknown` にも到達しない。

**数字は解析器の読みより先に扱う。**
SudachiPy は数字にも読みを付けるが1桁ずつ読むため
（`2024` → `ニレイニヨン`）、後段に置くと数値読み変換に到達しない。

> 当初は「読みが空か」を先頭に置き、数値読み変換とアルファベット読みを
> その後段に置いていた。実際には両方とも到達しないことが分かり、
> [#53](https://github.com/yama-shu/575-sns/issues/53) で優先順位を改めた。
...
SudachiDict-core は数字にも読みを付けるが、**1桁ずつ読む**。

```
入力 2024
  SudachiDict-core が返す読み: ニレイニヨン（5モーラ）
  本節の数値読み変換:         ニセンニジュウヨン（8モーラ）

入力 10
  SudachiDict-core が返す読み: イチレイ（4モーラ）
  本節の数値読み変換:         ジュウ（2モーラ）
```

`10` を「イチレイ」と読むのは日本語として誤りであり、モーラ数も合わない。
**数字は解析器の読みより先に数値読み変換を行う。**

> 当初はこの変換を解析器の読みの後段に置いていたため、
> SudachiDict-core を使う限り到達しなかった。
> [#53](https://github.com/yama-shu/575-sns/issues/53) で優先順位を改めた。

#### ASCII 以外の数字も数値として読む

`str.isdecimal()` は Unicode の十進数字すべてに真を返し、`int()` はそれらを変換する。
タミル数字 `௧௨௩` は 123 として `ヒャクニジュウサン` と読まれる。

数としては正しいが、日本語話者がそう読むとは限らない。
**現在の挙動として受け入れ、テストで固定する**（`test_ASCII以外の数字も数値として読まれる`）。
...
モーラ:          4   / 1  / 3  /   4     / 2  / 1 / 2   （計17）

候補A: 古池や | 蛙飛び込む | 水の音   → 5 / 7 / 5
   ズレ数 0 → 定型。区切りコストは「や（助詞）の直後」で 0、
   「の（助詞）の直前」で 1 となり、スコア 1

候補B: 古池 | や蛙飛び込む | 水の音   → 4 / 8 / 5
   ズレ数 2 → is_valid が false。候補にならない

→ 候補A が選ばれ、C単位のまま定型と判定される
```

> **本節は当初、この句が C単位では区切れないと記述していた。**
> `古池` を3モーラ、`飛び込む` を5モーラとしていたためだが、
> 実測では `フルイケ` = 4モーラ、`トビコム` = 4モーラであり、
> 5/7/5 に区切れる形態素境界が存在する。SudachiDict-core で確認した。

段階的探索（[§9](#9-段階的探索c単位--a単位)）が不要になるわけではない。
複合語が長く C単位の境界が粗すぎる句では、依然として A単位への切り替えが要る。
ただし**この句はその例として不適切だった**。
