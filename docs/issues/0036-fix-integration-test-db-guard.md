...
- [x] 接続先を `API_DATABASE_URL` とは別の環境変数で受ける
- [x] 接続先が**テスト用の DB であることを確かめる**（[下記](#db-名も確かめる)）
- [x] テスト用の DB を用意する手順を作る
- [x] CI を新しい環境変数に合わせる
- [x] README と `integration_test.go` の実行方法を直す
- [x] 詳細設計 04 §7 に実行の前提を書く

## 完了条件

- [x] 開発用の DSN を渡すと、**テストが実行される前に失敗する**
- [x] そのとき**開発用のデータが消えていない**
- [x] テスト用の DSN を渡すと、結合テストがすべて通る
- [x] 環境変数が未設定なら、これまでどおりスキップする
- [x] CI で結合テストが動き続けている（スキップされていないこと）

> **スキップされないことを機械で確かめる。** `CI` が設定されているのに接続先が
> 無ければ失敗させる。設定を落としたときに、緑のまま何も確かめていない状態にならない。
