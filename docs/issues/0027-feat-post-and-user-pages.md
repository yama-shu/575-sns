...
- [ ] `/@:handle` へ行けるようにする（[下記](#handle-の経路)）
- [ ] S-03 投稿詳細を実装する
- [ ] S-04 ユーザーページを実装する
- [ ] いいねの操作を実装する
- [ ] フォロー・解除を実装する
...
- [ ] 投稿者・判定・いいね数・日時が出る
- [ ] 未ログインでも閲覧できる
- [ ] 見えない投稿は 404 の画面になる（削除済み・ブロック・フォロワー限定）
- [ ] 投稿者名からユーザーページへ行ける
...
- [ ] もう一度押すと取り消され、数が減る
- [ ] 連打しても数がずれない
- [ ] 未ログインでは押せない（ログインへ誘導する）
- [ ] **JavaScript が無効でも押せる**（[下記](#操作はすべてフォームにする)）

### フォロー（[FR-04-02](../requirements/01-requirements.md#fr-04-交流)）

- [ ] フォロー・解除ができ、フォロワー数が変わる
- [ ] 自分のページには出ない
- [ ] 未ログインでは出ない（ログインへ誘導する）
- [ ] フォローするとフォロー中タイムラインに出るようになる

### ブロック（[FR-05-02](../requirements/01-requirements.md#fr-05-健全性)）

- [ ] ブロック・解除ができる
- [ ] ブロック中はその旨が分かり、投稿が0件になる
- [ ] **ブロックすると双方向にフォローが外れる**（BR-08）
- [ ] 解除できる（[#58](https://github.com/yama-shu/575-sns/issues/58) がそのために 404 にしていない）
