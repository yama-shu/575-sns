...
### 2026-08-16: 新サーバーを契約し、OS のメモリを実測した

[7章](#7-メモリ予算)が「本 ADR で唯一の未実測の前提」としていた **250 MiB は達成できなかった。**

| 状態 | `free -h` の `used` |
| --- | ---: |
| 契約直後（何も導入していない） | **413 MiB** |
| 不要なサービスを停止した後 | **392 MiB** |

停止したのは VPS で使わない4つ（`multipathd` 18 MiB、`ModemManager` / `udisks2` / `upower` 計 5 MiB）。

**棚卸しでは届かない。** 稼働中のサービスの合計は 77 MiB しかなく、残りはカーネルと slab である。
7章の 250 MiB は **1 GB のサーバーでの実測 363 MiB を基準にしていた**が、
**2 GB では OS の `used` がそれ自体大きくなる**ため、前提が成り立たない。

`fail2ban`（18 MiB）と `unattended-upgrades`（10 MiB）は残した。
公開サーバーに対する総当たり対策と自動セキュリティ更新であり、メモリより価値が上回る。

**予算への影響。** 7章は OS を 250 MiB として残り約 540 MiB としていた。
実測 392 MiB に置き換えると**残りは約 400 MiB**になる。
9章の「k3s + 575 + 監視が 2 GB に収まるか」の判定基準（`available` 400 MiB 以上）に対して、
**余裕はほぼ無い。** 第2期に監視を載せる段階で再測定し、載らなければ構成を見直す。

契約時に判明したその他の点。

| 項目 | 実際 |
| --- | --- |
| swap | **2 GB が既定で設定済み**。作成は不要だった |
| `snapd` | **最初から入っていない**。棚卸しの対象にならなかった |
| `fail2ban` | **最初から導入され稼働している** |
| OS | Ubuntu 24.04.3 LTS / kernel 6.8.0-90 / x86_64 |

### 2026-08-16: 575 を公開した

**https://575.ramen-oil.com**

| 項目 | 内容 |
| --- | --- |
| 構成 | docker compose + nginx（リバースプロキシ）+ certbot |
| 証明書 | Let's Encrypt。2026-11-14 まで。`certbot.timer` が自動更新 |
| イメージ | GHCR から pull（サーバー上でビルドしない） |

**k3s ではない。** 公開を後続作業の完了に紐づけないため、compose で先に公開した。
目標構成へは公開後に到達する。
...
| 状態 | `used` |
| --- | ---: |
| 基盤導入まで（Docker・nginx・certbot） | 458 MiB |
| 575 を起動した後 | 672 MiB |
| **575 の増分** | **214 MiB** |

7章は web 35 + api 3 + prosody 128 + postgres 40 = **206 MiB** と見積もっていた。
**実測 214 MiB とほぼ一致する。** ただし 7章の値は k3s 上で web と prosody を
2レプリカにした構成であり、compose の単一インスタンスとは前提が異なる。

公開後の `available` は 1.3 GiB である。

#### セキュリティグループが 80 / 443 を通していなかった

ConoHa の `default` セキュリティグループは **SSH のみを許可する。**
サーバー側で nginx が `0.0.0.0:80` を待ち受け、ufw も許可していたが、
外部からは無応答（拒否ではなくパケットの破棄）だった。

**`IPv4v6-Web` を追加して解決した。** 契約時に選ぶ項目であり、
次回の構築時に同じ箇所で止まらないよう記録する。

### 2026-08-16: oil_game を新サーバーへ移設した

**2サイトが同一ホストで HTTPS 公開されている状態になった。**

| ドメイン | 内容 |
| --- | --- |
| https://ramen-oil.com | oil_game（別プロジェクト） |
| https://575.ramen-oil.com | 575 |

[制約 D](#3-決定を左右する要因)（oil_game を巻き込んで落とさない）は満たしている。
移設中も 575 は稼働し続け、oil_game は DNS の切り替えまで旧サーバーで応答していた。

**旧サーバーは残す。** 自動更新は停止済みで、2026-09-11 に契約が終了する。
それまでは切り戻し先として保持する。

#### ポートが衝突した

oil_game は `8080` を使う（`docker-compose.yml` に記載）。
575 の api も既定で `8080` を公開していたため、後から起動した oil_game が失敗した。

**575 の api を `8081` に移した。** ブラウザは web としか通信せず
（[基本設計 01 §6](../design/basic/01-architecture.md#6-サービス間通信)）、
api のポート公開はホストからの確認用であるため、任意に変えてよい。

#### コンテナごとのメモリ実測値

| コンテナ | 実測 | 7章の見積り |
| --- | ---: | ---: |
| prosody | 126.2 MiB | 128 MiB（2レプリカ） |
| web | 76.1 MiB | 70 MiB（2レプリカ） |
| postgres | 46.8 MiB | 40 MiB |
| api | 3.4 MiB | 6 MiB（2レプリカ） |
| oil_game（app のみ） | **3.8 MiB** | 93 MiB |

**oil_game の見積りが大きく外れている。** 7章の 93 MiB は `dev`（node + vite の
開発サーバー）を含む値だった。本番では `app` のみを起動すれば足り、
nginx が静的ファイルを配信するだけのため 3.8 MiB で済む。

**約 89 MiB が予算に戻る。** OS の超過（250 → 392 MiB）で失った分の一部を相殺する。

ホスト全体では `used` 699 MiB、`available` 1.2 GiB である。
...
| 検証項目 | 結果 |
| --- | --- |
| 軽量 OS の選択で 250 MiB 前後に収まるか | **未達**。棚卸し後で 392 MiB（[上記](#2026-08-16-新サーバーを契約しos-のメモリを実測した)）。前提が成り立たなかった |
