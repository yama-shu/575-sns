...
更新日より前に解約すると、前払い済みの残期間を捨てることになる。

そこで、**更新日に合わせて移行する**。それまでの期間は 1 GB のまま構築を進める。

| 時期 | 環境 | やること |
| --- | --- | --- |
| **第1期**<br/>2026-08-09 〜 09-07 | ConoHa 1 GB<br/>+ ローカル | ドメイン取得 / Cloudflare・R2・Sentry の準備 / CI/CD 構築 / **K8s マニフェストをローカルの kind・k3d で作成し実証** / docker compose + Caddy で暫定公開し HTTPS 化 |
| **移行**<br/>2026-09-08 頃 | 両方（2〜3日並走） | 2 GB を新規契約（軽量 OS を選択）→ oil_game と 575 を移設 → 動作確認 |
| **第2期**<br/>2026-09-11 以降 | ConoHa 2 GB | 旧 1 GB を更新させない / k3s 構築 / 第1期のマニフェストを適用 / Prometheus + Grafana / 本番構成での性能再測定 |

並走期間の追加費用は約 79円（2〜3日分の時間課金）であり、無視できる。
...
**この 250 MiB は本 ADR で唯一の未実測の前提であり、[9章](#9-検証が必要な事項)の検証項目とする。**
...
| **移行作業そのもの** | 2026-09-08 頃に oil_game と 575 を新サーバーへ移す必要がある | 新サーバーで oil_game の正常稼働を確認してから旧サーバーを手放す。順序を逆にすると戻せない |
